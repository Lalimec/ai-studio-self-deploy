...
import { NanoBananaWebhookSettings, DownloadSettings, SeedreamProvider } from '../types';
...
interface GlobalSettingsModalProps {
    ...
    onSeedreamProviderChange: (provider: SeedreamProvider) => void;
}
...
const GlobalSettingsModal: React.FC<GlobalSettingsModalProps> = ({
    ...
    seedreamProvider, onSeedreamProviderChange
}) => {
            ...
            >
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t border-[var(--color-border-muted)]">
                        <h3 className="font-semibold text-[var(--color-text-light)] mb-2">Seedream Provider</h3>
                        <p className="text-sm text-[var(--color-text-dim)] mb-4">
                            Select the backend provider for Seedream model in Image Studio.
                        </p>
                        <div className="flex gap-2">
                            <button
                                onClick={() => onSeedreamProviderChange('higgsfield')}
                                className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${seedreamProvider === 'higgsfield'
                                    ? 'bg-[var(--color-primary)] text-[var(--color-text-on-primary)]'
                                    : 'bg-[var(--color-bg-muted)] text-[var(--color-text-dim)] hover:bg-[var(--color-bg-muted-hover)]'
                                    }`}
                            >
                                Higgsfield
                            </button>
                            <button
                                onClick={() => onSeedreamProviderChange('fal')}
                                className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${seedreamProvider === 'fal'
                                    ? 'bg-[var(--color-primary)] text-[var(--color-text-on-primary)]'
                                    : 'bg-[var(--color-bg-muted)] text-[var(--color-text-dim)] hover:bg-[var(--color-bg-muted-hover)]'
                                    }`}
                            >
