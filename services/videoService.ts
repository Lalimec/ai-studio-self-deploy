...
const VIDEO_GENERATION_ENDPOINT = 'https://n8n.cemil.al/webhook/5533f0bb-064a-4757-adcb-56793505fdf3/video-seedance-v1-pro/image-to-video';
const VIDEO_STATUS_ENDPOINT = 'https://n8n.cemil.al/webhook/5533f0bb-064a-4757-adcb-56793505fdf3/check/video-seedance-v1-pro/image-to-video';
...
const generateSingleVideo = async (image: ImageForVideoProcessing): Promise<string> => {
    if (!image.videoPrompt) {
        throw new Error(`Image ${image.filename} is not prepared for video generation.`);
    }

    const payload: { [key: string]: any } = {
        prompt: image.videoPrompt,
        image_url: image.src,
        aspect_ratio: 'auto',
        resolution: '720p',
        duration: '5'
    ...
    if (image.endSrc) {
        payload.end_image_url = image.endSrc;
    }

    // Step 1: Initiate video generation and get a request ID
    const initialResponse = await fetch(VIDEO_GENERATION_ENDPOINT, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const generateAllVideos = async (
    images: ImageForVideoProcessing[],
    onVideoGenerated: (filename: string, videoSrc: string) => void,
    onError: (errorMessage: string) => void
): Promise<void> => {
    const processTask = async (image: ImageForVideoProcessing) => {
        try {
            const videoSrc = await generateSingleVideo(image);
            onVideoGenerated(image.filename, videoSrc);
        } catch (error) {
            const message = error instanceof Error ? error.message : 'An unknown error occurred';
            onError(`Failed on ${image.filename}: ${message}`);
        }
    };
    
    // Process up to 8 video generations at a time
    await processWithConcurrency(images, processTask, 8);
};

export const generateSingleVideoForImage = async (image: ImageForVideoProcessing): Promise<string> => {
    return generateSingleVideo(image);
}
